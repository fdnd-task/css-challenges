[← overzicht](CHALLENGES.md)

---

# CSS challenge: Anchor positioning

---

## Samples

🎯 ⸺ de basics van anchor positining: [code](https://codepen.io/shooft/pen/EaNbexO) & [voorbeeld](https://codepen.io/shooft/pen/azBVabP) (CodePen 🎠)  
🎯 ⸺ anchor() function: [code](https://codepen.io/shooft/pen/ogYErPq) & [voorbeeld](https://codepen.io/shooft/pen/RNoQVWo) (CodePen 🎠)  
🎯 ⸺ anchor animeren: [code](https://codepen.io/shooft/pen/WboMjQz) & [voorbeeld](https://codepen.io/shooft/pen/jEVZmbK) (CodePen 🎠)  


---

## Oefeningen

🎯 ⸺ een popover die van kant wisselt indien nodig: [code](https://codepen.io/shooft/pen/xbRYmyM) & [voorbeeld](https://codepen.io/shooft/pen/zxoRyWJ) (CodePen 🎠)  

🎯 ⸺ moving focus: [code](https://codepen.io/shooft/pen/LEbQqNE) & [voorbeeld](https://codepen.io/shooft/pen/jEVZdWx) (CodePen 🎠)  

🎯 ⸺ input range met output: [code](https://codepen.io/shooft/pen/LEbQqNE) & [voorbeeld](https://codepen.io/shooft/pen/jEVZdWx) (CodePen 🎠)  

🎯 ⸺ scroll-spy: [code](https://codepen.io/shooft/pen/pvNaYrv) & [voorbeeld](https://codepen.io/shooft/pen/RNoQdGy) (CodePen 🎠)  

🎯 ⸺ carousel buttons en bolletjes: [code](https://codepen.io/shooft/pen/qEqxvGY) & [voorbeeld](https://codepen.io/shooft/pen/QwGQoQW) (CodePen 🎠)  

---

## Links

🎯 ⸺ [The CSS anchor positioning API](https://developer.chrome.com/docs/css-ui/anchor-positioning-api) (Una Kravets 🧑‍💻)  
🎯 ⸺ [Anchor positioning tool](https://chrome.dev/anchor-tool/) (Una Kravets 🧑‍💻)  

🎯 ⸺ [anchor()](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Values/anchor) (MDN)  
🎯 ⸺ [anchor-size()](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Values/anchor-size) (MDN)  

🎯 ⸺ [Introducing the popover API](https://developer.chrome.com/blog/introducing-popover-api/) (Una Kravets 🧑‍💻)  

🎯 ⸺ [shape(): A New Powerful Drawing Syntax in CSS](https://frontendmasters.com/blog/shape-a-new-powerful-drawing-syntax-in-css/) (Chris Coyier 🧑‍💻)  
🎯 ⸺ [border-shape: the future of the non-rectangular web](https://una.im/border-shape) (Una Kravets 🧑‍💻)  

🎯 ⸺ [Creating a scroll-spy with 2 lines of CSS](https://una.im/scroll-target-group/) (Una Kravets 🧑‍💻)  